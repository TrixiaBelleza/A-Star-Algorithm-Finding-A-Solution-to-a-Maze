...
Dapat RANDOMLY INITIALIZED YUNG MAP NOT READ FROM A FILE!!!
...
public class Main{
        ...
        *******************************************************/
        ...
        for(int i=0; i<size; i++){
            for(int j=0; j<size; j++){
                ...
                while((randValue == 2 && Gcount!=0) || (randValue==3 && Pcount!=0)){        //Monitors goal count and person count. Dapat kasi isang count lang yung goal and isa for person

                    rand = new Random();
                    randValue = rand.nextInt(3);                                            //min = 0, max = 3
                }
                if(i==size-1 && j==size-1 && Pcount==0){                                    //if last index and wala pa ring person, make last index the person
                    randValue = 3;
                }
                if(i==size-1 && j==size-1 && Gcount==0){                                    //if last index and wala pa ring goal, make last index the goal
                    randValue = 2;
                }
    ...
    private static double computeH(int XPos, int YPos, int goalXPos, int goalYPos){
        ...
        /***
            WHY MANHATTAN DISTANCE INSTEAD OF EUCLIDEAN DISTANCE
            ...
            Since the goal of the heuristic is to have closer approximation to a distance of two points,
            Manhattan distance formula is more appropriate to use since it computes for each paths taken towards the goal.
            In which, more approximate to the distance.

            Euclidean distance formula however, computes for the displacement only. In which, it's a bit farther 
            from the actual distance traveled towards the goal.
    ...
    private static boolean isDuplicate(ArrayList<State> stateList, State state){
        //checks if may state sa statelist na same XPOS and YPOS ng currentState
        for(int i=0; i<stateList.size(); i++){
    ...
    private static State Result(State state, String action, int goalXPos, int goalYPos, int sizeOfBoard){
        // Copy board
        // tapos yung bagong board yung imomodify
        //initialize ng bagong state
    ...
    private static boolean GoalTest(State state, int goalXPos, int goalYPos){
        // System.out.println("\nGoalTest: ");
        // System.out.println("PlayerX: " + state.getXPos() + "| PlayerY: " + state.getYPos() + " | GoalX: " + goalXPos + " | GoalY: " +goalYPos );
